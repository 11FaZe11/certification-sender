...
def add_text_to_pdf(input_path: str, output_path: str, text: str,
                    position: str = "bottom", offset: Tuple[float, float] = (0, 50),
...
def process_pdfs(excel_file, pdf_template, output_folder, name_col, phone_col,
                 position="bottom", font_size=24, font_name="Helvetica"):
...
def get_user_input_position() -> str:
    """Prompt user for text position."""
    print("\nAvailable positions:")
    ...
    while True:
        choice = input(f"Select position (1-{len(positions)}) or enter position name [default: bottom]: ").strip()
        if not choice:
            return "bottom"

        if choice.isdigit() and 1 <= int(choice) <= len(positions):
            return positions[int(choice) - 1]
        elif choice.lower() in positions:
            return choice.lower()
        else:
            print(f"Invalid choice. Please select 1-{len(positions)} or enter a valid position name.")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            excel_file = input("Enter Excel file name (default: test2.xlsx): ").strip() or "test2.xlsx"
            if not excel_file.lower().endswith('.xlsx'):
                excel_file += '.xlsx'

            pdf_template = input("Enter PDF template file name (default: pp.pdf): ").strip() or "pp.pdf"
            ...
            # Get styling options
            position = get_user_input_position()
